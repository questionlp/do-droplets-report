...
```bash
git clone https://github.com/questionlp/do-droplets-report.git
```
...
```bash
gunicorn report:app --reload
```

Once started, open a browser and browse to <http://127.0.0.1:8000/>. This should bring up the DigitalOcean Droplets Report web application.
...
`/tmp/gunicorn-do-droplets.sock` as the listener. If TCP socket is preferred, change the value to `IP:PORT` (replacing `IP` and `PORT` with the appropriate IP address of the interface and TCP port to listen to)
...
A template `systemd` service file is included in the repository named `gunicorn-do-droplets.service.dist`. That service file provides the commands and arguments used to start a Gunicorn instance to serve up the application. A copy of that template file can be modified and installed under `/etc/systemd/system`.

For this document, the service file will be installed as `gunicorn-do-droplets.service` and the service name will be `gunicorn-do-droplets`. The service file name, thus the service name, can be changed to meet your needs and requirements.
...
```bash
sudo systemctl enable gunicorn-do-droplets
sudo systemctl start gunicorn-do-droplets
```

Verify that the service started by running the following command:

```bash
sudo systemctl status gunicorn-do-droplets
```
...
```nginx
upstream gunicorn-do-droplets {
    server unix:/tmp/gunicorn-do-droplets.sock fail_timeout=0;
}
...
server {
    location / {
        ...
        proxy_redirect off;
        proxy_pass http://gunicorn-do-droplets;
    }
}
```
